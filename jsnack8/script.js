 ...
 */
 ...
 for ( let i = 0; i < 10; i++){
  ...
  const falseFirstNameIndex = Math.floor(Math.random() * nomi.length);

  const falseFirstName = nomi[falseFirstNameIndex].charAt(0).toUpperCase() + nomi[falseFirstNameIndex].slice(1);

  const falseLastNameIndex = Math.floor(Math.random() * cognomi.length);

  const falseLastName = cognomi[falseLastNameIndex].charAt(0).toUpperCase() + cognomi[falseLastNameIndex].slice(1);

  falseList.push(falseFirstName + " " + falseLastName);
